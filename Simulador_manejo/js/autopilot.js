...
export class Autopilot {
    ...
    generateWaypoints() {
        // ── CARRIL FÍSICO LaneBackward (Sentido Horario / Sentido contrario al jugador antiguo) ──
        // Offset = -2.5m a la derecha del sentido de marcha = +2.5m al interior del circuito
        // Trayectoria: recorre el perímetro en sentido HORARIO
        return [
            // Eje Z = -97.5, Conduciendo hacia el Oeste (-X)
            new THREE.Vector3( 95.0, 0.05, -97.5),
            new THREE.Vector3( 50.0, 0.05, -97.5),
            new THREE.Vector3(  0.0, 0.05, -97.5),
            new THREE.Vector3(-50.0, 0.05, -97.5),
            new THREE.Vector3(-95.0, 0.05, -97.5),

            // Eje X = -100, Conduciendo hacia el Norte (+Z) — carril a X = -97.5
            new THREE.Vector3(-97.5, 0.05, -95.0),
            new THREE.Vector3(-97.5, 0.05, -50.0),
            new THREE.Vector3(-97.5, 0.05,   0.0),
            new THREE.Vector3(-97.5, 0.05,  50.0),
            new THREE.Vector3(-97.5, 0.05,  95.0),

            // Eje Z = 100, Conduciendo hacia el Este (+X) — carril a Z = 97.5
            new THREE.Vector3(-95.0, 0.05,  97.5),
            new THREE.Vector3(-50.0, 0.05,  97.5),
            new THREE.Vector3(  0.0, 0.05,  97.5),
            new THREE.Vector3( 50.0, 0.05,  97.5),
            ...
            // Eje X = 100, Conduciendo hacia el Sur (-Z) — carril a X = 97.5
            new THREE.Vector3( 97.5, 0.05,  95.0),
            new THREE.Vector3( 97.5, 0.05,  50.0),
            new THREE.Vector3( 97.5, 0.05,   0.0),
            new THREE.Vector3( 97.5, 0.05, -50.0),
    ...
    update(delta) {
        ...
        if (this.navigationSystem) {
            ...
            // Llegada al Destino: seleccionar el siguiente automáticamente y continuar
            if (distToDest < 8.0 && this.currentWaypointIndex >= this.waypoints.length - 1) {
                this.navigationSystem.selectNextDestination();
                this.waypoints = this.navigationSystem.calculateRouteWaypoints(carPos);
                this.currentWaypointIndex = 0;
            }
        ...
        if (distToWP < this.waypointThreshold) {
            if (this.currentWaypointIndex < this.waypoints.length - 1) {
                ...
                // Recalcular ruta hacia el destino actual (ya cambiado si se llegó)
                this.waypoints = this.navigationSystem.calculateRouteWaypoints(carPos);
                this.currentWaypointIndex = 0;
            } else {
                this.currentWaypointIndex = 0; // Circuito cerrado sin navegación A*
            }
