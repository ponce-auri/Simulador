...
// ─────────────────────────────────────────────────────────────────────────────
// MÓDULO DE NAVEGACIÓN AUTÓNOMA Y PATHFINDING A*
// Administra el grafo urbano de la ciudad, selección de destinos,
// cálculo de rutas por carriles, visualización 3D y recálculo automático
// en caso de vías bloqueadas u obstáculos permanentes.
// ─────────────────────────────────────────────────────────────────────────────
...
export class NavigationSystem {
    ...
    _buildGraph() {
        // Generar 25 nodos (ix: 0..4, iz: 0..4)
    ...
    findPath(startPos, targetPos) {
        ...
        // Si no hay camino (por bloqueos extremos), fallback al camino directo
    ...
    calculateDualLaneWaypoints(nodePath, laneType = 'LaneForward') {
        ...
        const waypoints = [];
        // Ancho del carril = 5.0m, offset al centro de cada carril = 2.5m
        ...
        for (let i = 0; i < nodePath.length - 1; i++) {
            ...
            // Vector normal a la derecha del sentido de marcha
            const rightX = -dirZ;
            const rightZ =  dirX;

            // Coordenadas del carril correspondiente (LaneForward vs LaneBackward)
    ...
    calculateRouteWaypoints(carPos) {
        ...
        if (nodePath.length < 2) {
            // El coche ya está en el nodo de destino
            return [new THREE.Vector3(this.targetDestination.x, 0.05, this.targetDestination.z)];
        }

        // El vehículo autónomo siempre circula por LaneForward
        const waypoints = this.calculateDualLaneWaypoints(nodePath, 'LaneForward');
        
        // Waypoint final justo en el punto de destino
    ...
    markEdgeBlockedAndRecalculate(carPos) {
        ...
        // Buscar el siguiente nodo en el camino actual
